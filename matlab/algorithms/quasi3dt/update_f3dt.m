...
function [f_new_vec, sigma] = update_f3dt( f_old_vec,g,alpha,beta,gamma,v_vec,bv_vec,b_t_vec,d_t_vec,bu_vec,u_vec,...
    M,MT,grad_XYZ,gradT_XYZ,grad_T,gradT_T,L_XYZ,L_T,tol)
    ...
    TVpart_T = getGradient((b_t_vec - d_t_vec), grad_T, gradT_T, 1, 'backward');
    ...
    b = (2 * w.*g) - beta * TVpart_XYZ - gamma * TVpart_T - alpha * MT * (bu_vec - u_vec);

    % Assemble matrix (implemented via imfilter)
    % build up the left side of the equation
    Af = @(f_new_vec)cgCall3dt( f_new_vec, w, alpha, beta, gamma, M, MT, L_XYZ, L_T);
