...
function [ result ] = cgCall3dt( f_new_vec, w, alpha, beta, gamma, M, MT, L_XYZ, L_T)

    result = (2 * w .* f_new_vec) ...               % Data term
        + beta .* getLaplacian(f_new_vec,L_XYZ) ... % Total variation in x,y,z direction
        + gamma .* getLaplacian(f_new_vec,L_T) ...  % Total variation in t direction
