...
function [ f, E ] = admm3d( g,lambda,mu,omega,alpha,beta,gamma,tol,maxOuterIter,maxInnerIter,patchsize,p)
...
if numberOfVolumes > 1
    ...
    for vol = 2:numberOfVolumes
        startPos = (vol-1)*C*R*Z + 1;
        endPos = vol*C*R*Z;
        f_new = f_new + g(startPos:endPos);
    end
    f_new = f_new./numberOfVolumes;
else
    f_new = g;
end
...
% transposed gradient in t direction
...
for outerIter = 1:maxOuterIter
    ...
    if lambda > 0
        ...
        currentVolumes = vectorToVolumeStream(f_new_vec,1,C,R,Z);
        %for current=1:size(currentVolumes)
        
        [lookUpRow,lookUpColumn] = compute_Q3d(currentVolumes{1}, patchsize,p);
        
        
        M = speye(numberPixelVolume, numberPixelVolume) - sparse(lookUpRow,lookUpColumn,1,numberPixelVolume,numberPixelVolume);
        MT = speye(numberPixelVolume, numberPixelVolume) - sparse(lookUpColumn,lookUpRow,1,numberPixelVolume,numberPixelVolume);
    else
        M = speye(numberPixelVolume, numberPixelVolume);
        MT = speye(numberPixelVolume, numberPixelVolume);
    end
    ...
    for innerIter = 1:maxInnerIter
        ...
        [f_new_vec, sigma] = update_f3d( f_new_vec,g,alpha,beta,v_vec,bv_vec,...
            bu_vec,u_vec,M,MT,grad_XYZ,gradT_XYZ,L_XYZ,numberOfVolumes,tol);
        ...
        if nargout > 1
            ...
            E = [E; calculateEnergyFunction3d(f_new_vec, g, gradient, 1.345*sigma, mu, lambda, patchsize,p, C, R, Z, numberOfVolumes)];
        end
    ...
    if norm(f_old_vec - f_new_vec) / norm(f_old_vec) < tol
        f = vectorToVolumeStream(f_new_vec,1,C,R,Z);
        return;
    end
